...
for name, T_img in tqdm(T_img_dicts.items()):
    query_image_path = os.path.join(DATA_DIR, name, "T.png")
    ...
    if results['metadatas'] and results['metadatas'][0]:
        for metadata in results['metadatas'][0]:
            if 'character_name' in metadata:
                predicted_characters.add(metadata['character_name'])
    _names = list(predicted_characters)
    shuffle(_names)
    shuffled_dict = {key: info_dicts[key] for key in _names}
